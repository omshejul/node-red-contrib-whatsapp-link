module.exports = function(RED) {
    ...
    function WhatsappAdmin(config) {
        ...
        function SetStatus(WAStatus, color) {
            ...
            let msg = { payload: WAStatus };
            node.send(msg);
        };

        // Commands received for Whatsapp Admin.
        this.on('input', async function(msg, send) {
            if (msg.payload === "destroy") {
                ...
                if (node.client.clientType === "waWebClient") {
                    ...
                    SetStatus("Presence Available", "green");
                }
                ...
                if (node.client.clientType === "waWebClient") {
                    ...
                    setTimeout(() => {
                        SetStatus("Please Deploy all nodes to subscribe for messages", "yellow");
        ...
        if (node.client.clientType === "waWebClient") {
            ...
            node.client.on('group_join', async (notification) => {
                ...
                msg.chatID = msg.chat.id.user || `No ID Available`;
